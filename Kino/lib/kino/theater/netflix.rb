...
# @param file_name [String] movie list
# @note 
#  for right initialization you need a file with a file with films in such format:
...
# @example 
#   Kino::Netflix.new('movies.txt')
# @return [Kino::Netflix] Netflix class
# @see MovieCollection
module Kino
  class Netflix < MovieCollection
    ...
    # @note 
    #   If user would try to add < 0 he will get an error.
    #   add_money is a Cashbox method. Due to it's extended, add_money is a class method.
    #   I am using Money gem here, so when I add 20 dollars gem adds only 0.20 due its features. 
    #   Thats why it's necessary to multiply by 100. 
    #   So it is Netflix.add_money(coins * 100)
    # @example 
    ...
    # @param 
    #   key [Symbol] filter name 
    #   from [Symbol] another filter name
    #   agr [Integer, String] any argument 
    # @return [Proc] the result
    # @see show
    ...
    # The main netflix method. It shows a movie that matches the filter.
    # First of all, it divides user filters(custom) and system filters(internal)
    # Then it forms a variable 'list' with this filters.
    # Then it checks out if user have enough money to watch the movie and if he doesn't
    ...
    # Method for forming list variable.
    # @param 
    #   list [Array] filtered list
    #   custom_filters [Array] user's filter array
    # @see define_filter
    ...
    # @note 
    #   If movie doesn't exist or it's not in the list, user will get an error.
    # @example 
