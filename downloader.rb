...
class Downloader
  ...
  def id(movie)
    Tmdb::Find.movie(imdb_id(movie), external_source: 'imdb_id').first.id
  end
  ...
  def download_budget_for(movie)
    full_movie(movie).budget
  end
  ...
  def saving_to_hash(movie)
    ...
    @base[id_key] = [{:title => download_title_for(movie)}, {:poster => download_poster_for(movie)}, {:budget => download_budget_for(movie)}]
  end
